...
def spindle_add():
    ...
    return render_template("spindle_form.html", mode="add", today=today)


@app.route("/spindles/<record_id>/edit", methods=["GET", "POST"])
@login_required
def spindle_edit(record_id: str):
    rows = spindle_manager.all_records()
    ...
    return render_template("spindle_form.html", mode="edit", record=record)


@app.route("/spindles/<record_id>/delete", methods=["POST"])
@login_required
def spindle_delete(record_id: str):
...
def yedek_add():
    ...
    today = datetime.now().strftime(DATE_FORMAT)
    return render_template("yedek_form.html", mode="add", today=today)


@app.route("/yedeks/<record_id>/edit", methods=["GET", "POST"])
@login_required
def yedek_edit(record_id: str):
    rows = yedek_manager.all_records()
    record = next((r for r in rows if r["id"] == record_id), None)
    if not record:
        flash("Kayıt bulunamadı.", "danger")
    ...
    return render_template("yedek_form.html", mode="edit", record=record)


@app.route("/yedeks/<record_id>/delete", methods=["POST"])
@login_required
def yedek_delete(record_id: str):
